...
/**
 * @brief Widest text a batch number can produce, including a sign and a
 * terminator.
 *
...
/**
 * @brief Render @p value and return its length.
 *
 * Writes into @p out when given one, which is what lets the sizing pass and the
 * writing pass share this function.
 *
 * @param value Magnitude to render.
 * @param negative Whether to prefix a minus sign.
 * @param out Destination with room for kBatchNumberMax bytes, or nullptr to
 * only measure.
 * @return Number of bytes rendered, terminator excluded.
 */
...
/**
 * @brief Counts the bytes write_batch() would emit, so the caller can size its
...
/**
 * @brief Writes the bytes BatchCounter counted.
 */
class BatchAppender {
 public:
...
/**
 * @brief Batch-level fields.
 *
 * Pointers must outlive both passes; a zero length omits the optional mac.
 */
struct BatchHeader {
 ...
 *
 * A zero length omits the optional name or adv_data.
 */
struct BatchDevice {
  const char* mac;
  size_t mac_length;
...
/**
 * @brief Writes the advertisement batch the Signal K BLE provider endpoint
 * expects.
 *
 * Run once with a BatchCounter to size the buffer, then with a BatchAppender to
 * fill it. Both passes take this one path, so the field order and the
 * optional-field decisions cannot differ between them. The byte counts can:
 * BatchCounter::quoted and BatchAppender::quoted call different functions, and
 * so do the two hex(). json_quoted_length() against json_append_quoted() is
 * what the host tests pin, and the caller compares the two totals.
 *
 * @param sink BatchCounter to measure, or BatchAppender to write.
 * @param header Batch-level fields.
 * @param device_count Number of advertisements in the batch.
 * @param device_at Returns the i-th BatchDevice. Taking a callback rather than
 * a container keeps this free of Arduino and IDF types, so it can be tested on
 * the host.
 */
